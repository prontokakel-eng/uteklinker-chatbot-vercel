...
logToFile(`📂 Using test cases: ${casesPath}`);
...
const TAB_NAME = MODE === "backend" ? "TEST_TORT_PROD" : "TEST_TORTURE";
...
if (MODE === "backend") {
  console.log(`🚀 Running torture-v4 in BACKEND mode (direct chatPipeline, Sheet tab: ${TAB_NAME})`);
  logToFile(`🚀 BACKEND mode → ${targetUrl}`);
} else {
  console.log(`🚀 Running torture-v4 in LOCAL mode (API handler, Sheet tab: ${TAB_NAME})`);
  logToFile(`🚀 LOCAL mode`);
}
...
rows.push(["Timestamp","Mode","URL","Category","ExpectedLang","Input","GotLang","Expected","Reply","Status","FilterDebug","CaseRaw","LangMismatch","PreDetect","ChatDetect","Via"]);
...
(async () => {
  ...
  try {
    ...
    const TAB_NAME = MODE === "backend" ? (process.env.SHEET_TAB_NAME_PROD || "TEST_TORT_PROD") : (process.env.SHEET_TAB_NAME || "TEST_TORTURE");
    const tabIdEnv = MODE === "backend" ? process.env.SHEET_TAB_ID_TEST_TORT_PROD : process.env.SHEET_TAB_ID_TEST_TORTURE;
    const SHEET_TAB_ID = parseInt(tabIdEnv, 10) || 0;

    await sheets.spreadsheets.values.clear({ spreadsheetId, range: TAB_NAME });
    await sheets.spreadsheets.values.append({ spreadsheetId, range: TAB_NAME, valueInputOption: "RAW", requestBody: { values: rows } });
    ...
    logToFile(`✅ Sheets export ok → tab=${TAB_NAME} gid=${SHEET_TAB_ID}`);
  } catch (err) {
