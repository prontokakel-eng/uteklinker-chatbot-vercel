#!/usr/bin/env node
/**
 * generate-fixtures.mjs — builds /fix datasets from FAQ cache/Sheets
 *
 * GOAL
 *  - Read all FAQ Q/A across SE/EN/DA/DE from either:
 *      a) a cache JSON file (e.g. ./data/faq-cache.json)  OR
 *      b) a module that can fetch (e.g. ./lib/faq-data.js or ./lib/faq-sheets.js)
 *  - Emit augmented test inputs into /fix:
 ...
 *  - ESM-only, path-agnostic: ALL paths are provided via CLI flags — do not guess.
 *
 * USAGE (examples)
 *  node ./tests/generate-fixtures.mjs \
 *    --out ./fix \
 *    --langs SE,EN,DA,DE \
 *    --source.cacheJson ./data/faq-cache.json \
 *    --limit 0 \
 *    --seed 4242
 *
 *  # Or via modules (provide exactly one that can read):
 *  node ./tests/generate-fixtures.mjs \
 *    --out ./fix \
 *    --langs SE,EN,DA,DE \
 *    --source.module ./lib/faq-data.js \
 *    --symbol loadAllFAQSheets # exported fn returning [{id, lang, question, answer, category}]
 */
...
const OUT_DIR = argv.out ? String(argv.out) : './fix';
...
const SRC = {
  cacheJson: argv.source?.cacheJson,
  modulePath: argv.source?.module,
...
async function loadAllFAQSheets(){
  // a) From cache JSON
  if (SRC.cacheJson){
    const txt = await readFile(path.resolve(SRC.cacheJson),'utf8');
...
async function main(){
  ...
  await writeFile(path.join(OUT_DIR,'near.json'), JSON.stringify(near, null, 2));
  await writeFile(path.join(OUT_DIR,'local.json'), JSON.stringify(local, null, 2));
  await writeFile(path.join(OUT_DIR,'long.json'), JSON.stringify(long, null, 2));
  await writeFile(path.join(OUT_DIR,'negatives.json'), JSON.stringify(negatives, null, 2));

  console.log('Wrote:', { out: OUT_DIR, near: near.length, local: local.length, long: long.length, negatives: negatives.length });
}
