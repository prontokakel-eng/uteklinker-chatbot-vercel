...
def main():
    ...
    for i in range(len(df_se)):
        ...
        if not df.at[i, expected_cols[0]]:
            df.at[i, expected_cols[0]] = safe_translate(se_q, "Svenska", "Engelska", i+1, expected_cols[0])
        if not df.at[i, expected_cols[1]]:
            df.at[i, expected_cols[1]] = safe_translate(se_a, "Svenska", "Engelska", i+1, expected_cols[1])
    ...
    # Andra steget: översätt EN → DA och EN → DE
    for lang in ["DA", "DE"]:
        sheet = f"FAQ_{lang}"
        print(f"🔄 Bearbetar {sheet} ...")
        ...
        for i in range(len(df_se)):
            en_q = results["EN"].iloc[i]["question_en"]
            en_a = results["EN"].iloc[i]["answer_en"]

            if not df.at[i, expected_cols[0]]:
                df.at[i, expected_cols[0]] = safe_translate(en_q, "Engelska", lang, i+1, expected_cols[0])
            if not df.at[i, expected_cols[1]]:
                df.at[i, expected_cols[1]] = safe_translate(en_a, "Engelska", lang, i+1, expected_cols[1])
            if not df.at[i, expected_cols[2]]:
                df.at[i, expected_cols[2]] = "AI"
