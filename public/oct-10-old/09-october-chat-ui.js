...
// UPDATED: nya refs för flaggknappar
...
// === State ===
let lang = localStorage.getItem("ui.lang") || "SE";  // UPDATED: persistens
let lastDetectedLang = null;
let lastSave = null;

// UPDATED: helper för att toggla aktiv flagga + synka <select>
function setLanguage(newLang, { announce = true } = {}) {
  ...
  // synka select om den finns
  if (langSelect) {
    langSelect.value = lang;
  }

  if (announce) {
    showBanner(faqBanner, `🌐 Språk ändrat till ${lang}`);
  }
}
...
// === NEW: init språktillstånd från storage + sync UI
